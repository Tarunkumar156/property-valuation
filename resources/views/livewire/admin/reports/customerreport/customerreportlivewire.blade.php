    ...
    <div class="card shadow-sm">
        <div class="card-header text-white theme_bg_color p-1">
            <div class="d-flex flex-row bd-highlight">
                <div class="flex-grow-1 bd-highlight mt-1"><span class="h5">CUSTOMER REPORT</span>
                </div>
        ...
        <div class="card-body p-0">
            ...
            <div class="table-responsive">
                <table id="patientreport_id" class="table text-center table-hover m-0 p-0">
                    ...
                    <tbody>
                        @foreach ($customerreport as $key => $eachcustomer)
                            <tr>
                                <td>{{ $customerreport->firstItem() + $key }}</td>
                                <td>{{ $eachcustomer->uniqid }}</td>
                                <td>{{ $eachcustomer->name }}</td>
                                <td>{{ $eachcustomer->created_at->format('d-m-Y h:i A') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
